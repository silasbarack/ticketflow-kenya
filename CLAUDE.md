...
- **All tickets are sold on TicketFlow**: every listed event is `bookingMode = INTERNAL` with no `bookingUrl`. The frontend has no hard-coded event list and no link to outside sellers — add events to the DB (seed + a data migration for production), never as static frontend data.
...
- Branding: logo artwork lives in `frontend/public/brand/` (horizontal, stacked, mark, plus `-light` variants for dark backgrounds) and is rendered through `components/Logo.tsx`. `backend/assets/logo.png` is the same lockup on white, embedded in SMTP emails (CID attachment) and PDF tickets. The design system is `app/globals.css` (`tf-*` classes). Typography is Noto Sans only (`Noto Sans, Arial, sans-serif`) for body, headings and accents — don't reintroduce other faces. The hero phone mockup renders a real, scannable QR (`components/SiteQrCode.tsx`) linking to `/events` on the current origin.
