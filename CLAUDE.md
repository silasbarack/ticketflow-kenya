...
- **Payment trust boundary**: only `payments.service.ts` may set a payment to `SUCCESS`, and only on a verdict from Safaricom — either the M-Pesa callback (`POST /api/payments/mpesa/callback`) or a Daraja STK-query reconciliation (`reconcilePending`, which runs when a `PENDING` payment is read and is what makes local development work without a public callback URL) — or via the dev mock endpoint. Client-reported status is never trusted; polling may trigger a lookup but never decides its result. Payment success is what triggers commission recording and QR ticket generation.
